...
class MMCNNEncoderLayer(nn.Module):
    def __init__(
        ...
        self.a_conv = nn.Conv1d(
            input_size, output_size, 3, padding=1, dilation=dilation, bias=False
        )
        ...
        if input_size != output_size:
            self.a_skipconv = nn.Conv1d(
                input_size, output_size, 1, padding=0, dilation=dilation, bias=False
            )
            self.v_skipconv = nn.Conv1d(
                input_size, output_size, 1, padding=0, dilation=dilation, bias=False
            )
        else:
...
class CoSSM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        logger.info(f"dropout={str(dropout)} is not used in Mamba.")
        prev_input_size = input_size

        cnn_list = []
        mamba_list = []
        # logger.info(output_sizes)
...
class DepMamba(BaseNet):
    ...
    def feature_extractor(
        self, x, padding_mask=None, a_inference_params=None, v_inference_params=None
    ):
        xa = x[:, :, 136:]
        xv = x[:, :, :136]
