...
from models.mamba.selective_scan_interface import (
    selective_scan_fn,
    mamba_inner_fn,
    bimamba_inner_fn,
...
class Mamba(nn.Module):
    def __init__(
        ...
        if init_layer_scale is not None:
            ...
            self.v_gammagamma = nn.Parameter(
                init_layer_scale * torch.ones((d_model)), requires_grad=True
            )
    ...
    def forward(
        ...
        if a_inference_params is not None and v_inference_params is not None:
            a_conv_state, a_ssm_state = self.a_get_states_from_cache(
                a_inference_params, batch
            )
            v_conv_state, v_ssm_state = self.v_get_states_from_cache(
                v_inference_params, batch
            )
            if (
                a_inference_params.seqlen_offset > 0
                and v_inference_params.seqlen_offset > 0
            ):
                # The states are updated inplace
                a_out, _, _, v_out, _, _ = self.step(
                    a_hidden_states,
                    a_conv_state,
                    a_ssm_state,
                    v_hidden_states,
        ...
        if self.a_in_proj.bias is not None:
            a_xz = a_xz + rearrange(
                self.a_in_proj.bias.to(dtype=a_xz.dtype), "d -> d 1"
            )
        ## Video branch
        v_xz = rearrange(
        ...
        if self.v_in_proj.bias is not None:
            v_xz = v_xz + rearrange(
                self.v_in_proj.bias.to(dtype=v_xz.dtype), "d -> d 1"
            )
        ...
        if (
            self.use_fast_path
            and a_inference_params is None
            and v_inference_params is None
            ...
            if self.bimamba_type == "v1":
                ...
                if not self.if_devide_out:
                    a_out = F.linear(
                        rearrange(a_out + a_out_b.flip([-1]), "b d l -> b l d"),
                        self.a_out_proj.weight,
                        self.a_out_proj.bias,
                    )
                    v_out = F.linear(
                        rearrange(v_out + v_out_b.flip([-1]), "b d l -> b l d"),
                        self.v_out_proj.weight,
                        self.v_out_proj.bias,
                    )
                else:
                    a_out = F.linear(
                        rearrange(
                            0.5 * a_out + 0.5 * a_out_b.flip([-1]), "b d l -> b l d"
                        ),
                        self.a_out_proj.weight,
                        self.a_out_proj.bias,
                    )
                    v_out = F.linear(
                        rearrange(
                            0.5 * v_out + 0.5 * v_out_b.flip([-1]), "b d l -> b l d"
                        ),
                        self.v_out_proj.weight,
                        self.v_out_proj.bias,
                    )
            ...
            if a_conv_state is not None:
                ...
                a_conv_state.copy_(
                    F.pad(a_x, (self.d_conv - a_x.shape[-1], 0))
                )  # Update state (B D W)
            if v_conv_state is not None:
                # If we just take x[:, :, -self.d_conv :], it will error if seqlen < self.d_conv
                # Instead F.pad will pad with zeros if seqlen < self.d_conv, and truncate otherwise.
                v_conv_state.copy_(
                    F.pad(v_x, (self.d_conv - v_x.shape[-1], 0))
                )  # Update state (B D W)
            ...
            a_dt, a_B, a_C = torch.split(
                a_x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1
            )
            ...
            v_dt, v_B, v_C = torch.split(
                v_x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1
            )
    ...
    def step(
        ...
        if causal_conv1d_update is None:
            a_conv_state.copy_(
                torch.roll(a_conv_state, shifts=-1, dims=-1)
            )  # Update state (B D W)
            a_conv_state[:, :, -1] = a_x
            a_x = torch.sum(
                a_conv_state * rearrange(self.a_conv1d.weight, "d 1 w -> d w"), dim=-1
            )  # (B D)
            ...
            v_conv_state.copy_(
                torch.roll(v_conv_state, shifts=-1, dims=-1)
            )  # Update state (B D W)
            v_conv_state[:, :, -1] = v_x
            v_x = torch.sum(
                v_conv_state * rearrange(self.v_conv1d.weight, "d 1 w -> d w"), dim=-1
            )  # (B D)
        ...
        a_dt, a_B, a_C = torch.split(
            a_x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1
        )
        ...
        v_dt, v_B, v_C = torch.split(
            v_x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1
        )
        # Don't add dt_bias here
        ...
        if selective_state_update is None:
            ...
            a_ssm_state.copy_(
                a_ssm_state * a_dA + rearrange(a_x, "b d -> b d 1") * a_dB
            )
            ...
            v_ssm_state.copy_(
                v_ssm_state * v_dA + rearrange(v_x, "b d -> b d 1") * v_dB
            )
